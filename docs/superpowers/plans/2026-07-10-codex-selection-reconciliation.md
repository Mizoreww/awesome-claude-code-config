...
> **Adversarial-review amendment (authoritative over older task snippets below):** Name membership in `MANAGED_SKILLS` is an allowlist, not ownership proof. Both installers persist `~/.codex/.awesome-claude-code-config-managed-skills`; first-run migration adopts only expected `skills` lock sources whose Codex and canonical copies match, unchanged bundled copies, or an `obra/superpowers` repository. Reconciliation never scans generic `~/.agents/skills` children. Empty desired skill sets require confirmation before bulk removal. Ten retired `coding-foundations` names moved to a provenance-gated cleanup-only catalogue, and `handoff` was restored as a visible default-on Workflow choice. The original no-manifest/name-scan snippets in Tasks 2–4 are retained only as execution history and must not be reintroduced.
