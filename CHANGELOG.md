...
### Features
- **Cursor support (`cursor` branch).** This branch is a [Cursor](https://cursor.com)-targeted variant of the config, mirroring how the `codex` branch targets OpenAI Codex. The same instructions, coding rules, skills, MCP servers, hooks, and status line are re-expressed through Cursor's native mechanisms and installed into `~/.cursor/`:
  - `CLAUDE.md` → **`AGENTS.md`** (`~/.cursor/AGENTS.md`); paths and the memory system rewritten for `~/.cursor/`.
  - `rules/**/*.md` → **`.cursor/rules/*.mdc`** with Cursor frontmatter (`common-*` set `alwaysApply: true`; `python-*` / `typescript-*` / `golang-*` scoped by globs). Agent-orchestration guidance points at Cursor's real subagents (`explore`, `bugbot`, `security-review`, `browser-use`, …) and Plan mode instead of Claude-only named agents (`planner`, `tdd-guide`, `code-reviewer`, …) that Cursor does not ship.
  - `skills/*/SKILL.md` ported as Cursor skills (command-style skills set `disable-model-invocation: true`), plus a new `skills/update` Cursor self-updater.
  - `mcp/mcp-servers.json` → **`mcp.json`** (`{"mcpServers": {…}}`); zero-config `context7` + `playwright` ship active, secret-requiring `github` / `lark` are documented in `mcp.README.md`.
  - SessionStart lesson-loading → a **`sessionStart` hook** in `hooks.json` running `hooks/load-lessons.sh`.
  - **`statusline.sh`** (de-Claude-ified — the Anthropic 5-hour-usage block removed) wired via the `statusLine` entry in `cli-config.json`.
- **New installers `install-cursor.sh` / `install-cursor.ps1`.** A focused, idempotent, non-destructive installer that copies skills, rules, `AGENTS.md`, hooks, and the status line into `~/.cursor/`, and **merges** `mcp.json` / `cli-config.json` instead of overwriting. Supports `--dry-run` and a custom target via `--prefix <dir>` / `CURSOR_HOME`. Runs both from a local clone and **remotely** (`bash <(curl … cursor/install-cursor.sh)`), downloading the branch tarball into a temp dir when no clone is present, and **stamps the installed version** to `~/.cursor/.awesome-claude-code-config-version` so the `update` skill can detect upgrades. The Bash version bootstraps a static `jq` into `~/.cursor/bin/` for the JSON merges (and for the status line, which looks there too); the PowerShell port merges JSON natively.
- **Uninstall (`--uninstall` / `-Uninstall`) that reverts to the pre-install state.** The first install now records a verbatim **snapshot** of every config file it is about to overwrite (`~/.cursor/.awesome-claude-code-config.backup/`) plus a **manifest** of everything it installs (`~/.cursor/.awesome-claude-code-config.manifest`). Uninstall restores snapshotted originals, deletes installer-created files (rules, skills, hooks, status line, version stamp, snapshot, manifest), and **surgically un-merges only its own entries** from `mcp.json` / `hooks.json` / `cli-config.json` (removing a file only when nothing of yours remains). `lessons.md` is kept by default (`--purge-lessons` / `-PurgeLessons` drops a seeded one); `--force` / `-Force` skips the prompt and `--dry-run` previews. Configs from older builds with no manifest fall back to a conservative **degraded** mode that deletes only files byte-identical to what the repo ships.
- **New doc `docs/claude-main-to-cursor-migration.md`** — a user-facing Claude→Cursor concept-mapping guide (what installs where, how to install, what does *not* migrate 1:1). The internal working spec (`docs/cursor-migration-spec.md`) was folded into it and removed. The README / README.zh-CN gained a top-level **"Using with Cursor"** section, and the existing remote one-liner is now labeled **"Quick Start (Claude Code)"** so Cursor users aren't sent to the `~/.claude` installer.

### Design Rationale
- **Additive, never destructive.** The Cursor entry points sit *alongside* the existing Claude installer and files (`install.sh`, `CLAUDE.md`, `settings.json`, `mcp/`) rather than replacing them, so the branch is a superset. The installer backs up every config file it overwrites (`*.bak.<timestamp>`), merges `mcp.json` / `cli-config.json` / `hooks.json` so user-configured servers and hooks survive re-runs, resolves the status-line command and the `statusline.sh` / `load-lessons.sh` scripts to the chosen install dir (so `--prefix` / `CURSOR_HOME` works at runtime, not just for file layout), only touches the rule/skill files it ships (leaving user-added ones intact), and seeds `lessons.md` solely when absent.
- **Secrets stay out of the active `mcp.json`.** Strict JSON has no comments and Cursor re-spawns every listed server each session, so credential-gated servers are documented in `mcp.README.md` instead of shipped with placeholder tokens — avoiding broken servers spawning with bogus credentials.
- **A working core over brittle feature-parity.** The installer deliberately targets the core pieces with small, readable functions rather than recreating the full interactive menu of the Claude installer.

### Notes & Caveats
- **`jq` is required for the JSON merges** (and the status line). The Bash installer auto-downloads a static binary into `~/.cursor/bin/`; if that fails it prints manual install instructions and the status line degrades to a one-line notice rather than crashing. When `jq` is unavailable *and* a target JSON already exists, the merge is skipped (existing file left intact) instead of clobbering it.
- **`sessionStart` may not inject context.** Cursor's `sessionStart` hook is documented for session setup/audit and may not feed stdout into the agent context the way Claude Code's `SessionStart` did. Lessons still reach the agent reliably via `~/.cursor/AGENTS.md` (which references `lessons.md`); the hook ships anyway as it is forward-compatible.
- **Two update-style skills coexist** (`update-config` and the new `update`); `update` is the canonical Cursor self-updater. Both now invoke `install-cursor.sh` / `install-cursor.ps1` (the Cursor installers) — not the Claude `install.sh` — and rely on the version stamp above. `install-cursor.ps1` could not be executed on the Linux dev box (no PowerShell) — it is modeled on the working `codex` PowerShell installer and is best-effort; its remote-mode, version-stamping, and uninstall logic mirror the verified Bash paths.
- **Uninstall does not undo post-install edits.** It reverts what the installer did, not changes you made afterward: a rule/skill file you edited in place is removed (it is still installer-owned), while edits to *merged* JSON are preserved because un-merge is key-scoped. The Bash uninstall path is covered by a 38-assertion test matrix (clean install→uninstall, pre-existing-file restore, dry-run, `--purge-lessons`, and degraded mode); the PowerShell equivalent is unverified on this box.
